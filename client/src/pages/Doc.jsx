...
import { getFancyResult, getResult, placeBet, placeFancyBet } from "../reducer/resultSlice";
...
const handleTestEndpoint = async () => {
  ...
  try {
    ...
    // Add query parameters from testParams
    if (selectedEndpoint.params) {
      selectedEndpoint.params.forEach(param => {
        if (param.name !== 'key' && testParams[param.name]) {
          params[param.name] = testParams[param.name];
        }
      });
    }

    // Prepare body data for POST requests
    const bodyData = {};
    if (selectedEndpoint.method === 'POST' && selectedEndpoint.body) {
      selectedEndpoint.body.forEach(param => {
        if (testParams[param.name]) {
    ...
    // Dispatch based on which API category is selected
    switch(openMain) {
      case "Cricket API":
        if (selectedEndpoint.name === "Get Cricket Matches") {
          response = await dispatch(getCricketMatches(params)).unwrap();
        } else if (selectedEndpoint.name === "Match Betting Data") {
          response = await dispatch(getCricketBettingData({ gameid: testParams.gameid, key:testParams.key })).unwrap();
        } else if (selectedEndpoint.name === "Get Tennis Matches") {
          response = await dispatch(getTennisMatches(params)).unwrap();
        } else if (selectedEndpoint.name === "Tennis Betting Data") {
          response = await dispatch(getTennisBettingData({ gameid: testParams.gameid, key:testParams.key })).unwrap();
        } else if (selectedEndpoint.name === "Get Soccer Matches") {
          response = await dispatch(getSoccerMatches(params)).unwrap();
        } else if (selectedEndpoint.name === "Soccer Betting Data") {
          response = await dispatch(getSoccerBettingData({ gameid: testParams.gameid, key:testParams.key })).unwrap();
        }
        break;
      ...
      case "Online game API":
        if (selectedEndpoint.name === "Get Game Details") {
          ...
          response = await dispatch(getActiveProviders(params)).unwrap();
        } else if (selectedEndpoint.name === "Launch Game") {
          response = await dispatch(launchGame(bodyData)).unwrap();
        } else if (selectedEndpoint.name === "Get User Balance") {
          response = await dispatch(getUserBalance(bodyData)).unwrap();
        } else if (selectedEndpoint.name === "Set User Balance") {
          response = await dispatch(setUserBalance(bodyData)).unwrap();
        } else if (selectedEndpoint.name === "Bet History") {
          response = await dispatch(getBetHistory(bodyData)).unwrap();
        }
        break;
        
      case "Cricket Result":
        if (selectedEndpoint.name === "Place Bet") {
          response = await dispatch(placeBet(bodyData)).unwrap();
        } else if (selectedEndpoint.name === "Get Result") {
          response = await dispatch(getResult(bodyData)).unwrap();
        } else if (selectedEndpoint.name === "Place Fancy Bet") {
          response = await dispatch(placeFancyBet(bodyData)).unwrap();
        } else if (selectedEndpoint.name === "Get Fancy Result") {
          response = await dispatch(getFancyResult(bodyData)).unwrap();
        }
        break;
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'dark' : ''}`}>
      {/* <Header /> */}
      <div className={`flex h-screen ${classes.bg}`}>
        {/* Sidebar */}
           ...
           {/* Base URL Info - Updated to show both URLs */}
            {/* <div className={`mt-8 p-4 ${classes.bgSecondary} rounded-xl border ${classes.border}`}>
              ...
              <div className="space-y-2">
                ...
                <div className={`flex items-center gap-2 ${classes.card} p-2 rounded-lg border ${classes.border} group hover:border-purple-500 transition-colors`}>
                  <code className={`text-xs flex-1 ${classes.text} font-mono`}>Result API: {API_CONFIG.resultbaseUrl}</code>
                  <button 
                    onClick={() => handleCopy(API_CONFIG.resultbaseUrl)}
                    className={`p-1 ${classes.hover} rounded transition-all hover:scale-110`}
                  >
                    {copied ? <Check size={14} className="text-green-600" /> : <Copy size={14} className={classes.textMuted} />}
                  </button>
